...
void shuffle_partition(vector<int> &labels){
    ...
    size_t half_size = round(labels.size()/2);
...
void bipartition(
    ...
    if(label_row.size()>2){
        ...
        int num_label1 = 0;
        int num_label2 = 0;
        for (size_t i=0 ; i<label_row.size() ; i++){
            rnd_label = rand() % 2;
...
vector<vector<int> > recursive_step(
    ...
    for (int i=0; i<repetitions; i++){
        ...
        while (current_block <= max_number_blocks){
           ...
           // sprintf(outputString,"fprintf('%s: splitting block = %i\\n');",log_prefix_global,current_block);
            ...
            for (size_t i=0; i<labels_final_col.size(); i++){
            ...
            for (size_t l=0; l<labels_final_row.size(); l++){
            ...
            if(indices_row.size()>1 && indices_col.size()>1){
                ...
                for (size_t j=0; j<indices_row.size(); j++){
                    vector<int> aux;
                    for (size_t k=0; k<indices_col.size(); k++){
                        aux.push_back(adjacency_matrix[indices_row[j]][indices_col[k]]);
                    }
                    sub_matrix.push_back (aux);
                }
                //calculate degress
                for (size_t i_rows = 0; i_rows < indices_row.size(); i_rows++) {
                    for (size_t j_cols = 0; j_cols < indices_col.size(); j_cols ++) {
    ...
    for (size_t k=0; k<col_labels_out.size();k++){
        col_labels_out[k]=(col_labels_out[k]-min_block);
    }
    for(size_t m=0; m<row_labels_out.size();m++){
